...
`examples/board` is the demo, and it is also the kit's acceptance test.
...
examples/board/        the demo app — a workspace member, depends on fjkit
...
```bash
uv run fastapi dev examples/board/app/main.py    # the demo
uv run pytest                                    # kit suite + demo suite
uv run ruff check
uv run fjkit check examples/board/app/templates  # closed-vocabulary gate
...
`examples/board/tests/test_conventions.py` enforces 1, 2 and the naming rules.
