...
- **伺服器端一個位元組都沒變。** `FjkitConfig.style` 仍然是每個 process 一個包，
  ...
  stylesheet。
- **手寫 JS——需要人類授權的那一類，由使用者直接指定。** 15 行，全在 `examples/fjkit-demo` 的
...
- **不進 wheel，進 app。** 品質預算把使用者端 JS 封在 htmx + basecoat；
  ...
  數量級，不是預算裡放得下的東西。所以它 vendored 在 `examples/fjkit-demo/app/static/`，
...
pragma。`examples/fjkit-demo` 撞不到只是因為 app 模板不會討論顏色。
