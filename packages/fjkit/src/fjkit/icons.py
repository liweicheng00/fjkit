...
Only the icons a page calls reach the response, so shipping the whole set costs
package size and nothing on the wire.

The set is loaded from JSON on first use rather than imported as a Python
module, for two reasons: `import fjkit` stays fast for an app that renders no
icons on its hot path, and a version bump is a one-line diff in a data file
instead of a 20,000-line one in the source tree.
...
def path(name: str) -> Markup:
    ...
    Raises `KeyError` on an unknown name rather than rendering an empty box,
    because a silently missing icon is the kind of bug that ships. The
    suggestion earns its place: among 1,767 names, `arrow-right` and
    `arrow-right-circle` are a coin flip from memory.
    """
    icons = _icons()
    try:
