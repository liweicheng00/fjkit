...
listed here, and nothing else, is allowed to ship inside the wheel. Two of
them reach every page. The others do not, and that is the whole reason each
is a separate pin — see `HTMX_JSON_ENC_VERSION` and `PLOTLY_VERSION`.
"""
...
#: moved every extension out of the core repository, and this one tracks the
#: extension rather than the core it plugs into.
#:
#: **It is not loaded by any page unless that page asks for it** — `form_scripts()`
#: in `ui/form.html`, the same page-level opt-in `chart_scripts()` uses for
#: Plotly. CHARTER §7 budgets what a page downloads *by default*, and the answer
#: has to stay "htmx and Basecoat", so 1,012 bytes that only some forms need
#: cannot go in the shell.
HTMX_JSON_ENC_VERSION = "2.0.3"

#: Plotly's *basic* bundle — scatter, bar and pie — drawn by `fjkit.charts`.
#: The full `plotly.js-dist-min` is 4.85 MB of 3D, maps, financial and
#: statistical traces most dashboards never draw; the basic one is 1.1 MB, and
#: picking the bundle is the only size knob Plotly offers without a build step.
#:
#: **Loaded by no page unless that page asks for it** — `chart_scripts()`, the
