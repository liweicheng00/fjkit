...
The browser is given one opaque, signed, HttpOnly cookie. The access token, the
refresh token and the app's claims stay in a `SessionStore` on the server. This
is the token-handler shape (a BFF): a page can carry a session without any
JavaScript reading, storing or attaching a credential, so an htmx app needs no
`htmx:configRequest` hook, no refresh-and-retry glue, and no token in `<body>`.

What the cookie carries is a random 32-byte id and an HMAC of it. Signing an
already-unguessable id is not about secrecy — it is so that a forged or
corrupted cookie is rejected without a round trip to the store.

The trade this makes, stated plainly: a cookie is an ambient credential, so
CSRF comes back. `fjkit.auth.csrf` is why that is handled here rather than left
to the app.
"""
...
#: What the user is told, per reason. Two, because they are different events:
#: one is "you were never signed in here", the other is "you were, and it ran
#: out" — and only the second is worth interrupting someone about.
_FLASH_TEXT = {
...
def _full_path(request) -> str:
    """The path with its query string, so a filtered view survives the trip.

    `request.url.path` alone drops it, and the user comes back from the login
    form to an unfiltered list wondering what happened to their search.
    """
...
def _is_navigation(request) -> bool:
    """Whether a person is looking at this, rather than a program.

    Two signals, either one is enough. `Sec-Fetch-Mode: navigate` is the
    purpose-built one — every current browser sends it when the address bar is
    moving, and nothing else sends it at all. `Accept` is the fallback for the
    browsers that predate it: a navigation asks for `text/html`, while curl,
    `fetch()` and an HTTP client all send `*/*`, which expresses no preference
...
class CookieSpec:
    ...
    `httponly` is not a field. The entire point of this plugin is that no
    script can read the credential; a knob to turn that off would only ever be
    ...
    #: site, so following a link into the app keeps you logged in, while a
    #: cross-site write gets nothing. `"none"` gives that protection up and
    #: leaves the `Origin` check standing alone — only for an app embedded in
    #: another site's iframe.
...
class AuthPlugin:
    ...
    Every request costs one store read. That is the price of being able to
    revoke a session by deleting a row, and of keeping refresh tokens off the
    wire entirely — neither is possible with a self-contained encrypted cookie.

    Routes see none of it. Put `Depends(auth.required)` on a router and its
    handlers keep the signatures they had; the session is on
    `request.state.auth`, and in every template as `session`.
    """
    ...
    def __init__(
        ...
        *,
        ...
        #: Optional, and injected rather than imported. A plugin depending on
        #: another plugin is a rule this kit does not have; the app registers
        #: both and hands one to the other, so auth still works without it and
        #: flash is still useful to routes that have nothing to do with auth.
        self.flash = flash
    ...
    async def issue(self, request: Request, response: Response, credentials: Any) -> Session:
        ...
        Takes `request` as well so it can drop the caller's previous session —
        an id that survives a login is a session-fixation hole. This is the one
        moment the id changes; a refresh deliberately keeps it (see
        `_refresh`).

        The login route is therefore `async def`, which is correct for it: it is
        doing network I/O. Every rendering route stays `def`.
        """
        ...
        # So a login that renders rather than redirects still sees the session
        # it just created.
    ...
    async def required(self, request: Request) -> Session:
        """Dependency. Refuses an anonymous request.

            router = APIRouter(dependencies=[Depends(auth.required)])

        Returned as well as raised, so a handler that wants the session can
        take it as a parameter instead of reading `request.state`.
        """
    ...
    async def load(self, request: Request) -> Session | None:
        """The session for this request, refreshed if it was about to expire.

        Called by the middleware. Raises `NotAuthenticated` only for a session
        that existed and could not be renewed — an absent or unreadable cookie
        ...
        try:
            ...
            # rather than 500 every request until someone clears their cookies.
            await self.store.delete(sid)
            return None
    ...
    async def _refresh(self, sid: str, session: Session) -> Session:
        """Renew the token once, however many requests noticed at the same time.

        Self-guarding: a session that is not near expiry, or a source that
        cannot renew, comes straight back out. Callers do not have to know
        which case they are in.

        The session id does not change here. Rotating it would mean several
        concurrent swaps each returning a different `Set-Cookie`; the last one
        wins, the others become orphans, and the user is logged out by the very
        mechanism meant to keep them in.
        """
        if not self._expiring(session):
            return session

        if not hasattr(self.source, "refresh"):
            # Reported here rather than at startup, because at startup it is not
            # yet true: a source with no `refresh` and no `expires_at` is an
            # ordinary configuration, and warning about it would train people to
            # ignore the warning that matters. Once, so a busy app does not
        ...
        try:
            # Read again now that the lock is held. Between this request's read
            # and this line another one may have finished renewing, and the
            # token we are holding is single-use: spending it a second time is
            # what makes an upstream with rotation revoke the whole family.
            payload = await self.store.get(sid)
            ...
            try:
                ...
                # Only the upstream saying no ends a session. A store that
                # failed a moment ago would otherwise log every user out over a
                # blip, which is a far worse failure than one retried request.
                await self.store.delete(sid)
    ...
    def _expiring(self, session: Session) -> bool:
        """True when the token is inside the leeway, or already past it.

        `expires_at is None` means the source never said, so there is nothing to
        act on and the session is left alone.
        """
    ...
    def _handle(self, request: Request, exc: Exception) -> Response:
        """One reply builder for both entry points.
        ...
        middleware stack: an error raised in middleware never reaches a
        registered handler. Sharing the function is what stops the two paths
        from drifting into different answers.
        """
        if isinstance(exc, CsrfRejected):
            # Not a redirect to the login page: the browser is where it should
            # be, and this request is the thing that is wrong.
            return Response(status_code=403)

        # Three callers, three answers — the same question `@render` asks about
        # what to put on the wire, asked about how to refuse.
        #
        # 401 everywhere is the tidy reading of the spec and wrong for exactly
        # one of the three: a plain navigation has no way to act on it, so it
        # lands on a blank page. The other two get 401, and the htmx one still
        # moves — see below.
        vary = {"vary": "HX-Request, Accept, Sec-Fetch-Mode"}
        # `safe="/"` and nothing else. Leaving `?`, `=` or `&` raw would let the
        # value's own query string break out and become extra parameters of the
        # login URL, so `next` would arrive truncated at the first `?`.
        ...
        if is_htmx(request):
            ...
            #
            # It has to move the whole page. A 303 would be followed by htmx's
            # own fetch and the login form swapped into whatever element the
            # click targeted; a bare 401 would swap nothing, because htmx
            # ignores a 4xx body, and the click would appear to do nothing.
            ...
            # A script, curl, another service. Under `render_mode="auto"` this
            # is the caller a route answers in JSON, so refusing it with a
            # redirect to an HTML login form would be answering a question it
            # did not ask — and following that redirect would hand it a login
            # page with a 200, which reads as success.
            #
            # No `WWW-Authenticate`: the field is required by RFC 9110 and every
            # registered scheme is wrong here. `Basic` would make the browser
            # open its own credential dialog, which is precisely the login form
        ...
        # Both branches above end in a new page load, which is the one thing
        # `HX-Trigger` cannot survive: the document it would fire into is gone.
        # A cookie is what crosses that gap.
    ...
    async def dispatch(self, request: Request, call_next: Any) -> Response:
        ...
        try:
            if plugin._read_cookie(request) is not None:
                ...
                # that carries no session has nothing for CSRF to steal, and
                # refusing it would break endpoints that are meant to be open.
...
def safe_next(value: str | None, fallback: str = "/") -> str:
    """The `next=` value, or the fallback if it is not ours to return to.

    The plugin only ever *writes* a path here, so nothing it produced can point
    off-site. What arrives back is a query string, which anyone can type — and
    a login page that redirects to whatever it is handed is an open redirect:
    a link to your own domain, your own login form, and a hop to the attacker's
    site at the exact moment someone has just typed their password.

    So the rule is positional rather than a blocklist. One leading slash, and
    not two: `//evil.example.com` is a protocol-relative URL, which a browser
    reads as a different site entirely, and it is the case a naive
