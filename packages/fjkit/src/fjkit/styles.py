...
carrying one entry point and no CSS. That indirection is not a design flourish:
a Python extra can only pull in *another distribution*, and cannot change a byte
of the wheel it belongs to. So `fjkit[nova]` leaves a marker behind, and this is
where fjkit finds it.

Config wins over marker, always. An explicit value in code is someone saying it
on purpose; a marker is something the environment was installed with.
"""
...
def installed_packs() -> tuple[StylePack, ...]:
    ...
    A name that is not one of ours is ignored rather than fatal: a stale marker
    left behind by an uninstall should not stop an app from booting, and the
    pack it names would not be in the wheel to serve anyway.
    """
...
def resolve_style(style: StylePack | Literal["auto"]) -> StylePack:
    """Turn a configured value into the pack the shell will link.
    ...
    match installed_packs():
        ...
        case (pack,):
            return pack
        case packs:
            # Silently picking one would make the page's appearance depend on
            # metadata scan order, which is the kind of bug that survives a
            # bisect. Two packs installed is a question only a human can answer.
            listed = ", ".join(packs)
