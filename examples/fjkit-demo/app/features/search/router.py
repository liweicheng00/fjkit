...
One id, many fragments: a pick is not that shape. What the click produces is an
id, and the panels that care each need a *different* query. So `search_select`
answers with its own region and broadcasts, and the panels fetch themselves.

Both halves obey the same rule: **the fragment a route answers with must not
subscribe to the event that route raises.** It is already being replaced in band;
hearing the event too would fetch it a second time for the same click, and on a
fragment that is its own target it never stops. So the matches table is the reply
to `search_select` and hears only `task-changed`, and the detail panel is the
reply to `search_advance` and hears only `task-selected`.
"""
...
def select_task(service: ServiceDep, task_id: int, q: str = "") -> MatchesResponse:
    ...
    One round trip doing both halves. The table is this button's `hx-target`, so
    ...
    browser, which is the only place it is state.

    The event is declared on the decorator, and `@render` resolves `task_id` out
    of this handler's own parameters. The detail is an *object*: htmx passes an
...
def facets_fragment(service: ServiceDep, q: str = "") -> FacetsResponse:
    ...
    priority — which is the clearest thing on the page about subscription being
    each fragment's own decision.
    """
    matches = service.search(q)
