...
`run()` is sync, so Starlette runs it in the threadpool. It stays on the class
rather than moving to a module of its own: every step reads and writes the same
dictionary under the same lock, so a free function would have to be handed the
store's internals to be correct.
"""
...
from app.schemas.jobs import KINDS, Job, JobKind, JobState
